;The tests are mainly taken from Scheme 9 from Empty Space.
;I have deleted a few tests that are wrong or not fit for me .

(define (test exp res)
  (if (not (equal? exp res))
      (error "test failed" (list exp '=> res))
      "passed!"))
...
(test (odd? 1) #t)
(test (odd? 2) #f)
(test (odd? 123456780) #f)
(test (odd? 123456781) #t)
